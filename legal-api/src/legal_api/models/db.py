...
def print_versioning_info():
    ...
    try:
        ...
        if current_service:
            ...
            current_app.logger.error(f'\033[31mService: {current_service}, db versioning={current_versioning}\033[0m')
    except Exception as err:
        # Don't crash if something goes wrong
        current_app.logger.error(f'\033[31mUnable to determine versioning type: {err}\033[0m')
...
class VersioningProxy:
    ...
    def _switch_versioning(cls, previous, current):
        ...
        current_app.logger.error(f'\033[31mVersioning changed: {previous} -> {current}\033[0m')
